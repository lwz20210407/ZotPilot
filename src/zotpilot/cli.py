...
def _print_formula_parser_comparison(report: dict) -> None:
    ...
    print(f"  Supported clusters:       {report.get('supported_cluster_count', 0)}")
    print(f"  Review clusters:          {report.get('review_cluster_count', 0)}")
    print(f"  OCR fallback clusters:    {report.get('ocr_fallback_cluster_count', 0)}")
...
def cmd_index_formulas(args):
    ...
    if "write_blocked" in result or "write_ready" in result:
        write_status = (
            "blocked"
            if result.get("write_blocked")
            else "ready"
            if result.get("write_ready")
            else "not ready"
        )
