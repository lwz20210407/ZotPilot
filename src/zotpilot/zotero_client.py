...
_WEAK_TRANSLATED_PLUGIN_FILENAME_RE = re.compile(
    r"\s等\s+-\s+\d{4}\s+-.*-\d{5,}\.pdf$",
    re.IGNORECASE,
)
_CJK_RE = re.compile(r"[\u4e00-\u9fff]")
_NUMBERED_FORMULA_LINE_RE = re.compile(r"\(\s*(?:[A-Za-z]\.)?\d+(?:\.\d+)*\s*\)")
...
def is_likely_bilingual_or_translated_pdf(path: Path | str | None) -> bool:
    """Return true for PDF files likely generated by bilingual translation plugins."""
    if path is None:
        return False
    filename = str(path).replace("\\", "/").rsplit("/", 1)[-1]
    return bool(_TRANSLATED_PDF_MARKER_RE.search(filename))


def has_translation_plugin_filename_shape(path: Path | str | None) -> bool:
    """Return true for weak translation-plugin filename shapes.

    This is a ranking hint only. Some Zotero libraries use Chinese-style
    "Author 等 - year - title-id.pdf" names for original PDFs, so this shape
    must not by itself make formula backfill skip the whole item.
    """
    if path is None:
        return False
    filename = str(path).replace("\\", "/").rsplit("/", 1)[-1]
    return bool(_WEAK_TRANSLATED_PLUGIN_FILENAME_RE.search(filename))


def pdf_content_translation_risk_score(path: Path | str | None, *, title: str = "", max_pages: int = 6) -> float:
    """Return a best-effort content risk score for translated/bilingual PDFs.

    The score is intentionally conservative for Chinese original papers: CJK
    text is only penalized when the Zotero title itself is not Chinese. Repeated
    numbered formula lines are a stronger bilingual-layout signal.
    """
    if path is None:
        return 0.0
    pdf_path = Path(path)
    ...
    try:
        ...
        for page_index in range(pages_to_check):
            ...
            for block in page.get_text("blocks"):
                ...
                if not _looks_like_numbered_formula_line(block_text):
                    continue
                normalized = re.sub(r"\s+", "", block_text)
                if normalized in seen_lines:
                    duplicate_numbered_lines += 1
                else:
                    seen_lines.add(normalized)
        title_has_cjk = bool(_CJK_RE.search(title or ""))
        cjk_ratio = cjk_chars / max(total_chars, 1)
        score = duplicate_numbered_lines * 20.0
        if not title_has_cjk and cjk_ratio >= 0.20:
            score += min(200.0, cjk_ratio * 500.0)
        elif not title_has_cjk and cjk_ratio >= 0.02:
            # Original theses and publisher PDFs may include Chinese cover,
            # copyright, or abstract pages. Treat low CJK density as a hint,
            # not enough by itself to skip the original PDF.
            score += min(10.0, cjk_ratio * 200.0)
        return score
    finally:
        doc.close()
...
def _pdf_filename_translation_risk_score(path: Path | str | None) -> float:
    if path is None:
        return 0.0
    score = 0.0
    if is_likely_bilingual_or_translated_pdf(path):
        score += 1000.0
    if has_translation_plugin_filename_shape(path):
        score += 5.0
    return score
...
class ZoteroClient:
            ...
            FROM (
                SELECT
                    ...
                    ROW_NUMBER() OVER (
                        ...
                        ORDER BY
                            CASE
                                WHEN ia.path LIKE '%双语%' OR ia.path LIKE '%对照%'
                                  OR ia.path LIKE '%翻译%' OR ia.path LIKE '%译文%'
                                  OR ia.path LIKE '%全文翻译%' OR ia.path LIKE '%中英%'
                                  OR ia.path LIKE '%英中%'
                                  OR lower(ia.path) LIKE '%bilingual%'
                                  OR lower(ia.path) LIKE '%translated%'
                                  OR lower(ia.path) LIKE '%translation%'
                                  OR lower(ia.path) LIKE '%dual-language%'
                                  OR lower(ia.path) LIKE '%dual_language%'
                                  OR lower(ia.path) LIKE '%zh-en%'
                                  OR lower(ia.path) LIKE '%en-zh%'
                                  OR lower(ia.path) LIKE '%cn-en%'
                                  OR lower(ia.path) LIKE '%en-cn%'
                                THEN 1 ELSE 0
                            END,
                            CASE WHEN EXISTS (
                                SELECT 1
                                FROM itemAttachments cache_ia
                                JOIN items cache_items ON cache_ia.itemID = cache_items.itemID
                                WHERE COALESCE(cache_ia.parentItemID, cache_ia.itemID)
                                      = COALESCE(ia.parentItemID, ia.itemID)
                                  AND lower(COALESCE(cache_ia.path, '')) LIKE '%mineru%'
                                  AND lower(COALESCE(cache_ia.path, '')) LIKE '%' || lower(items."key") || '%'
                                  AND (
                                      lower(COALESCE(cache_ia.path, '')) LIKE '%.zip'
                                      OR lower(COALESCE(cache_ia.contentType, '')) LIKE '%zip%'
                                  )
                            ) THEN 0 ELSE 1 END,
                            CASE
                                WHEN ia.path LIKE '% 等 - % - %-_____%'
                                THEN 1 ELSE 0
                            END,
    ...
    def _load_citation_key(self, item_key: str) -> str:
        """Load one BetterBibTeX citation key without scanning the full BBT table."""
        if not self.bbt_db_path.exists():
            return ""
        conn = sqlite3.connect(_sqlite_uri(self.bbt_db_path), uri=True)
        conn.row_factory = sqlite3.Row
        try:
            ...
            return str(row["citationKey"]) if row is not None else ""
        finally:
            conn.close()
    ...
    def resolve_original_pdf_path(
        ...
        *,
        ...
        """Choose the lowest-risk original PDF among an item's PDF attachments."""
        attachments = self._pdf_attachments_for_item(item_key) if attachments is None else attachments
        if not attachments:
            return fallback_path
        if len(attachments) == 1:
            return attachments[0][1]
        cache_keys = self._mineru_cache_pdf_keys_for_item(item_key)
        if cache_keys:
            cache_matched_attachments = [
                pdf_path for attachment_key, pdf_path in attachments
                if attachment_key in cache_keys
            ]
            if len(cache_matched_attachments) == 1:
                return cache_matched_attachments[0]
        ranked: list[tuple[float, str, Path]] = []
        content_risk_max_pages = 1 if cache_keys else 6
        for attachment_key, pdf_path in attachments:
            score = _pdf_filename_translation_risk_score(pdf_path)
            score += pdf_content_translation_risk_score(
                pdf_path,
                title=title,
                max_pages=content_risk_max_pages,
            )
            if cache_keys:
                score += -4.0 if attachment_key in cache_keys else 2.0
            if fallback_path is not None and pdf_path == fallback_path:
                score -= 0.1
            ranked.append((score, attachment_key, pdf_path))
        ranked.sort(key=lambda row: (row[0], row[1]))
        return ranked[0][2] if ranked else fallback_path

    def _pdf_attachments_for_item(self, item_key: str) -> list[tuple[str, Path]]:
        ...
        for row in rows:
            path = self._resolve_pdf_path(row["path"], row["linkMode"], row["attachmentKey"])
            if path is not None and path.exists():
                attachments.append((row["attachmentKey"], path))
        return attachments

    def _mineru_cache_pdf_keys_for_item(self, item_key: str) -> set[str]:
        conn = sqlite3.connect(_sqlite_uri(self.db_path), uri=True)
        conn.row_factory = sqlite3.Row
        try:
            rows = conn.execute(
        ...
        for row in rows:
            ...
            match = _MINERU_CACHE_KEY_RE.search(path)
            if match:
    ...
    def mineru_cache_paths_for_item(
        ...
        *,
        ...
        different PDF attachment key are ignored. This lets formula indexing use
        the selected original PDF's cache directly instead of scanning the whole
        Zotero storage tree.
        """
        selected_pdf_key = ""
...
"""
    ...
    def _row_to_item(self, row, citation_keys: dict[str, str]) -> ZoteroItem:
        ...
        fallback_pdf_path = self._resolve_pdf_path(
            row["path"],
            row["linkMode"],
            row["attachmentKey"]
        ) if row["attachmentKey"] else None
        item_key = row["itemKey"]
        pdf_path = self.resolve_original_pdf_path(
            item_key,
            title=row["title"],
            fallback_path=fallback_pdf_path,
        ) if fallback_pdf_path is not None else None
        return ZoteroItem(
            item_key=item_key,
            title=row["title"],
            authors=row["authors"],
    ...
    def get_item(self, item_key: str) -> ZoteroItem | None:
        """Get a specific item by key using item-scoped queries."""
        conn = sqlite3.connect(_sqlite_uri(self.db_path), uri=True)
        conn.row_factory = sqlite3.Row
        try:
            base = conn.execute(
                """
                SELECT itemID, "key" AS itemKey, dateAdded
                FROM items
                WHERE "key" = ?
                  AND libraryID = ?
            ...
            field_rows = conn.execute(
                ...
                WHERE itemData.itemID = ?
                  AND fields.fieldName IN ('title', 'date', 'publicationTitle', 'DOI')
                """,
                (item_id,),
            ).fetchall()
            fields = {row["fieldName"]: row["value"] for row in field_rows}
            creator_rows = conn.execute(
                """
                SELECT creators.lastName, creators.firstName, itemCreators.orderIndex
                FROM itemCreators
                JOIN creators ON itemCreators.creatorID = creators.creatorID
        ...
        year_text = fields.get("date", "") or ""
        try:
            year = int(year_text[:4]) if year_text[:4] else None
        except ValueError:
            year = None
        authors = "[No Author]"
        if creator_rows:
            first = creator_rows[0]
            first_name = first["firstName"] or ""
            suffix = f", {first_name[:1]}." if first_name else ""
            authors = f"{first['lastName']}{suffix}"
            if len(creator_rows) > 1:
                authors = f"{first['lastName']} et al."
        attachments = self._pdf_attachments_for_item(item_key)
        fallback_pdf_path = attachments[0][1] if attachments else None
        pdf_path = self.resolve_original_pdf_path(
            item_key,
            title=title,
            fallback_path=fallback_pdf_path,
        ...
        return ZoteroItem(
            ...
            year=year,
            pdf_path=pdf_path,
            citation_key=self._load_citation_key(item_key),
            publication=fields.get("publicationTitle", ""),
            doi=fields.get("DOI", ""),
            tags="; ".join(row["name"] for row in tag_rows),
            collections="; ".join(row["collectionName"] for row in collection_rows),
            date_added=base["dateAdded"] if "dateAdded" in base.keys() else "",
        )
