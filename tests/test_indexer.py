...
class TestFormulaBackfill:
    ...
    def test_estimate_formula_backfill_blocks_high_density_unnumbered_structured_cache(self, tmp_path):
        from zotpilot.feature_extraction.formula_ocr import FormulaCandidate
        from zotpilot.indexer import Indexer
        from zotpilot.models import ZoteroItem
        ...
        pdf_path = tmp_path / "paper.pdf"
        pdf_path.write_bytes(b"%PDF-1.4")
        item = ZoteroItem("DOC1", "Formula-heavy paper", "Auth", 2024, pdf_path)
        candidates = [
            FormulaCandidate(
                page_num=1 + index // 8,
                bbox=(0, index * 10, 100, index * 10 + 8),
                raw_text=rf"x_{{{index}}}=y_{{{index}}}+1",
                confidence=0.95,
                equation_number_status="unnumbered",
                latex=rf"x_{{{index}}}=y_{{{index}}}+1",
                source="mineru_content_list",
            )
        ...
        with patch("zotpilot.feature_extraction.formula_ocr.extract_formula_candidates", return_value=candidates):
            result = indexer.estimate_formula_backfill(candidate_preview_limit=0)

        audit = result["results"][0]["candidate_audit"]
        assert audit["numbered_count"] == 0
        assert audit["unnumbered_count"] == 81
        assert audit["equation_number_warnings"] == [
        ...
        assert result["candidate_quality_blocking_papers"][0]["candidate_quality_severity"] == (
            "structured_cache_numbering"
        )
    ...
    def test_estimate_formula_backfill_allows_distant_numbering_restarts(self, tmp_path):
        from zotpilot.feature_extraction.formula_ocr import FormulaCandidate
        from zotpilot.indexer import Indexer
        from zotpilot.models import ZoteroItem
        ...
        pdf_path = tmp_path / "paper.pdf"
        pdf_path.write_bytes(b"%PDF-1.4")
        item = ZoteroItem("DOC1", "Chaptered paper", "Auth", 2024, pdf_path)
        candidates = [
            FormulaCandidate(
                page_num=page,
                bbox=(0, index * 10, 100, index * 10 + 8),
        ...
        assert audit["duplicate_equation_number_count"] == 0
        assert audit["repeated_equation_number_sections"] == ["(4)", "(6)", "(7)", "(8)"]
        assert audit["repeated_equation_number_section_count"] == 4
        assert audit["equation_number_warnings"] == []
        assert result["candidate_quality_blocking_paper_count"] == 0
    ...
    def test_estimate_formula_backfill_ignores_sparse_mixed_prefix_missing_gap(self, tmp_path):
        from zotpilot.feature_extraction.formula_ocr import FormulaCandidate
        from zotpilot.indexer import Indexer
        from zotpilot.models import ZoteroItem
        ...
        pdf_path = tmp_path / "thesis.pdf"
        pdf_path.write_bytes(b"%PDF-1.4")
        item = ZoteroItem("DOC1", "Mixed numbering thesis", "Auth", 2024, pdf_path)
        candidates = [
            FormulaCandidate(
                page_num=page,
                bbox=(0, index * 10, 100, index * 10 + 8),
        ...
        audit = result["results"][0]["candidate_audit"]
        assert audit["missing_equation_number_total"] == 0
        assert audit["equation_number_sequence_breaks"] == []
        assert audit["equation_number_warnings"] == ["mixed_equation_number_prefixes"]
        assert result["candidate_quality_blocking_paper_count"] == 0
