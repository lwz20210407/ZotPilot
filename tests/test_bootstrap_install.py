...
class TestZotpilotBinaryResolution:
    ...
    def test_pip_win(self, tmp_path, monkeypatch):
        ad = tmp_path / "AppData" / "Roaming"
        sc = ad / "Python" / "Python313" / "Scripts"
        sc.mkdir(parents=True)
        (sc / "zotpilot.exe").touch()
        ...
        with (
            ...
            patch.object(_p, "system", return_value="Windows"),
...
class TestEnsureZotpilot:
    ...
    def test_uv_tool_install_succeeds(self, tmp_path):
        with (
            patch("shutil.which", return_value=None),
            patch.object(_rm, "SKILL_DIR", tmp_path),
            patch("platform.system", return_value="Darwin"),
            patch.object(_rm, "_find_zotpilot_after_pip", return_value=None),
...
class TestHandleRegister:
    def test_register_uv(self):
        ...
        with (
            patch.object(_rm, "_ensure_uv", return_value="uv"),
            patch.object(_rm, "_ensure_zotpilot", return_value=None),
            patch.object(_rm, "_uv_args", return_value=["uv"]),
            patch("subprocess.run", side_effect=fr) as mr,
        ):
    ...
    def test_register_pip(self):
        with (
            patch.object(_rm, "_ensure_uv", return_value="uv"),
            patch.object(_rm, "_ensure_zotpilot", return_value=["/p/zotpilot"]),
            patch("subprocess.run", return_value=MagicMock(returncode=0)) as mr,
        ):
...
class TestMainDelegation:
    ...
    def test_other_delegated(self):
        with (
            ...
            patch.object(_rm, "_uv_args", return_value=["uv"]),
            patch("subprocess.run", return_value=MagicMock(returncode=0)) as mr,
            patch("sys.argv", ["run.py", "status", "--json"]),
            patch("sys.exit"),
        ):
            _rm.main()
